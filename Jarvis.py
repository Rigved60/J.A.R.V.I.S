...
def Start_Jarvis():
    pg.press('esc')
    wish()
    play("D:\\Rigved\\alexa course\\My Alexa projaect\\Main Jarvis\\Jarvis_audio.mp3")
    ...
    while True:
        ...
        userA = r.recognize_google(audio).lower()
        userA = userA.lower()
        speak("OK")
        ...
        if 'chrome' in userA:
            ...
            song = userA.replace('play', '')
            E.say('playing ', userA + song)
            print("playing . .....")
            E.runAndWait()
            kit.playonyt(song)
        ...
        elif 'who is' in userA:
            p = userA.replace('who is', '')
            info = wiki.summary(p, 3)
            print(info)
            speak(info)
            E.runAndWait()

        elif 'what is' in userA:
            ...
            speak("You have to get up at 7o clock, brush and get to breakfast. After that you have your breakfast and then you go for the school assignments given.. Then you have lunch and go play game. After completing it you go for the JARVIS conference and after that, watch Tv. That is how your schedule looks today sir")
            E.runAndWait()
...
while True:
    ...
    for (x, y, w, h) in faces:
        ...
        if (accuracy < 100):
            ...
            speak(('User Authentication Failed..Try Again'))
